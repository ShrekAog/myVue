			...
			<div class="search-input" v-if="status.list.length >= 1" :class="[{ focus: isFocus }]">
				<div class="all-el">
					<div class="logo" @click="isEngine = !isEngine, isFocus = true">
						<el-image class="engine-logo" fit="cover"
							:src="status.list[status.defaultEngine.index].resource.path"></el-image>
					</div>
					<input @focus="isFocus = true, isEngine = true" v-model="status.defaultEngine.txt" class="main-input"
						type="text" placeholder="请输入内容" :disabled="loading"
						@keyup.enter="search(status.list[status.defaultEngine.index])" />
					<div class="search-logo" @click="search(status.list[status.defaultEngine.index])">
						<el-image class="search-img" fit="cover"
							:src="getAssetsImageFile('index/engine-icon/search.png')"></el-image>
					</div>
				</div>
