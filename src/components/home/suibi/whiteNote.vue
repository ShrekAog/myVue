	...
	>	
		...
		<div class="main-container">
			<div class="flexCenter">
				<el-input class="area" @click.stop @focus="isFocus = true" v-model="publishObj.content" type="textarea" resize="none" :show-word-limit="true" :autosize="{minRows:3,maxRows:7}"></el-input>
				<div class="upimg" @click.stop="isFocus = true">
					<el-upload
						class="flexCenter"
						:auto-upload="false"
				...
				<div class="upvideo" @click.stop="isFocus = true">
					<el-upload
						class="flexCenter"
						:auto-upload="false"
						:show-file-list="false"
			...
			<div class="el-box" :class="{'active':isFocus}" @click.stop>
				...
				<div class="flexCenter">
					...
					<el-button type="primary" @click="addNote" style="margin-left: 10px;">发表</el-button>
				</div>
			</div>
			<emojiPicker 
			@click.stop 
		...
		<div class="resource-box">
			...
			<div class="upResource" v-if="progressSchedule > 0">
				<el-progress :percentage="progressSchedule"></el-progress>
			</div>
...
<script setup>
	...
	import { showElMsg,sendNote,upNoteResource,delNoteById } from '@/api/api'
	import { useStore } from 'vuex'
import loginVue from '../login.vue'
	const store = useStore();
	const maxListBl = ref(false);
	const progressSchedule = ref(0);//上传进度条
	...
	const emoji = (emoji) => {
		console.log(emoji.i)
		publishObj.value.content += emoji.i;
	}
	const photoChange = (file) => {
		if(list.value.length >= 9){
			if(!maxListBl.value){
				showElMsg("warning","照片最多1~9张");
				maxListBl.value = true;
			}
			setTimeout(() => {maxListBl.value = false},3000)
			return;
		}
	...
	const resetStatus = () => {
		...
		if(publishObj.value.content.length > 0){
			isFocus.value = true;
		}
	...
	const addNote = async () => {
		if(publishObj.value.content?.length >= 1 || list.value.length >= 1){
			loading.value = true;
			publishObj.value.userId = store.state.user.id;
			let note = await sendNote(publishObj.value);
			if(list.value.length <= 0){
				emit('update:visible',false);
			}else{
				let noteId = note.data.data;
				let formData = new FormData();
				list.value.forEach(file => {
					formData.append("files",file);
				})
				formData.append("username","user" + store.state.user.id);
				formData.append("userId",store.state.user.id);
				formData.append("noteId",noteId);
				showElMsg("info","正在上传...")
				
				let upRes = await upNoteResource(formData,onUploadPro => {
					progressSchedule.value = Math.round((onUploadPro.loaded * 100) / onUploadPro.total);
				})
				if(upRes == null){
					await delNoteById(note.data.data);
				}
